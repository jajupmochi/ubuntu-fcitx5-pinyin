...
export class InputPanel extends GObject.Object {
    ...
    _init(params) {
        ...
        // 咪咪 8-frame running-dog loader state (driven by a GLib timer)
        this._dogTimer = 0;
        this._dogFrame = 0;
        this._dogIndex = -1;
    ...
    setLookupTable(label, table, visible) {
        ...
        // Detect the cloud-loading spinner slot (fcitx emits ◐◓◑◒ while waiting on
        // cloud pinyin). That slot becomes 咪咪; a GLib timer animates the 8 frames.
        const _spin = {'◐' : 1, '◓' : 1, '◑' : 1, '◒' : 1};
        let dogIdx = -1;
        for (let i = 0; i < lookupTable.length; i++) {
            ...
            if (_spin[_t]) {
                dogIdx = i;
                lookupTable[i].text = ''; // frame class applied by _paintDog()
            } else {
    ...
    // ---- 咪咪 running-dog loader (8-frame gallop, 75ms/frame) ----
    _setDog(idx) {
        this._dogIndex = idx;
        if (idx < 0) {
            this._stopDog();
        ...
        this._paintDog(); // paint one frame right away
        if (!this._dogTimer) {
            this._dogTimer =
    ...
    _paintDog() {
        ...
        for (let k = 0; k < 8; k++)
            item.remove_style_class_name('kimpanel-dog-' + k);
        item.add_style_class_name('kimpanel-dog-' + this._dogFrame);
    }
    _stopDog() {
        if (this._dogTimer) {
            GLib.source_remove(this._dogTimer);
            this._dogTimer = 0;
        }
        this._dogIndex = -1;
    }
    ...
    updateFont(textStyle) {
        // Size + family come from the 'font' gsetting (currently 'LXGW WenKai 14').
        // Do NOT append a hardcoded font-size here — it would override the gsetting
        // (a stray '; font-size: 16pt;' was why the requested 14pt never took effect).
